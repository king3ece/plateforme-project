...
- **Bon Pour** :
  ...
  - Front : formulaire, onglet “Bon pour” dans _Mes demandes_ + onglet de validation dedie (actions valider/rejeter/a corriger, affichage des lignes, commentaires).
...
- **Onglets unifies** : la page _Mes demandes_ et la page _Validation_ affichent maintenant quatre onglets (FDM / Bon pour / RFDM / DDA) partageant la mise en forme (tableaux, cartes, dialogues de detail reutilisant `RequestDetailContent`).
...
- Le front consomme l API via `axiosInstance` (`http://localhost:9091/api`) avec token JWT depuis `localStorage`.
